...
function MyPage() {
  ...
  const removeSearch = async (id: string) => {
    const res = await fetch(`https://ec2-54-180-133-86.ap-northeast-2.compute.amazonaws.com/api/db/wish?act=remove&id=${id}&email=${sessionStorage.getItem('Email')}`, {
      method: 'GET',
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const getUserDB = async () => {
    const res = await fetch(`https://ec2-54-180-133-86.ap-northeast-2.compute.amazonaws.com/api/db/view?email=${sessionStorage.getItem('Email')}`, {
      method: 'GET',
      headers: {
          "Content-Type": "application/json",
      },
